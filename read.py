...
def readExport(
    ...
    * k-points must lie on uniform grid
    ...
    * NOTE: K indices increase from [-Kmax, -Kmax+1, ..., Kmax]
    ...
    """
    with open('{}/index.xml'.format(exroot)) as f:
        for line in f:
    ...
    # rearrange k-points into rank 2 array
    k_l2 = k_a2.tolist()
...
def fileSort(s):
    """Key to sort grid.{k} and wfc.{k} in order of k-points in listed in k_a2.
    """
    _, n = s.split('/')[-1].split('.')
    return int(n)
...
def getKpts(
    ...
    """returns k-points used for QuantumEspresso run."""
    try:
